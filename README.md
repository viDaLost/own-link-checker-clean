...
- потоковая проверка до 10 000 URL за один запуск;
...
`done` поступают по мере выполнения проверки. Большой запуск автоматически
разделяется клиентом на пакеты до 200 URL, поэтому функция остаётся в пределах
ограничений Vercel, а общий лимит одного запуска составляет 10 000 ссылок.
